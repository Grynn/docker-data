...
_start:
	...
	message:
		db 0x0A
		db 'Welcome to grynn/data', 0x0A
		db 'If you see this message, your data-container is ready to provide persistent-storage.', 0x0A
		db 0x0A
		db 'To backup the content of this container (image?):', 0x0A
		db ' $ docker run --volumes-from data --rm -v $(pwd):/backup busybox tar cvf /backup/data.tar /data', 0x0A
		db 0x0A
		db 'To restore the content of this container (image):', 0x0A
		db ' $ docker run --volumes-from data --rm -v $(pwd):/backup busybox tar xvvf /backup/data.tar -C /', 0x0A
		db 0x0A
